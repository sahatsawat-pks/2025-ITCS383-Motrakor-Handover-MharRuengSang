const pool = require('../db');

// GET CART
const getCart = async (req, res) => {
  const user_id = req.user.id;
  try {
    const result = await pool.query(
      `SELECT cart.id, games.id as game_id, games.title, games.price, games.cover_image
       FROM cart
       JOIN games ON cart.game_id = games.id
       WHERE cart.user_id = $1`,
      [user_id]
    );
    res.json(result.rows);
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: 'Server error' });
  }
};

// ADD TO CART
const addToCart = async (req, res) => {
  const user_id = req.user.id;
  const { game_id } = req.body;
  try {
    // Check if game exists
    const game = await pool.query(
      'SELECT * FROM games WHERE id = $1', [game_id]
    );
    if (game.rows.length === 0) {
      return res.status(404).json({ message: 'Game not found' });
    }

    // Check if already in cart
    const existing = await pool.query(
      'SELECT * FROM cart WHERE user_id = $1 AND game_id = $2',
      [user_id, game_id]
    );
    if (existing.rows.length > 0) {
      return res.status(400).json({ message: 'Game already in cart' });
    }

    // Check if already purchased
    const purchased = await pool.query(
      'SELECT * FROM purchases WHERE user_id = $1 AND game_id = $2',
      [user_id, game_id]
    );
    if (purchased.rows.length > 0) {
      return res.status(400).json({ message: 'Game already purchased' });
    }

    await pool.query(
      'INSERT INTO cart (user_id, game_id) VALUES ($1, $2)',
      [user_id, game_id]
    );
    res.status(201).json({ message: 'Game added to cart' });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: 'Server error' });
  }
};

// REMOVE FROM CART
const removeFromCart = async (req, res) => {
  const user_id = req.user.id;
  const { gameId } = req.params;
  try {
    await pool.query(
      'DELETE FROM cart WHERE user_id = $1 AND game_id = $2',
      [user_id, gameId]
    );
    res.json({ message: 'Game removed from cart' });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: 'Server error' });
  }
};

module.exports = { getCart, addToCart, removeFromCart };